...
const FONT_PATH = '/fonts/'; // Path in /public directory or accessible via web server
...
const loadAndRegisterFont = async (doc: jsPDF, fontFileName: string, fontFamilyName: string, fontWeight: 'normal' | 'bold'): Promise<boolean> => {
    try {
        const response = await fetch(`${FONT_PATH}${fontFileName}`);
        if (!response.ok) {
            console.error(`Failed to fetch font ${fontFileName}: ${response.statusText}. Status: ${response.status}`);
            return false;
        }
        ...
        console.error(`Error loading or registering font ${fontFileName}:`, error);
        return false;
    }
};
...
const SP_SPACING_AFTER_LOGO = 6; 

const SP_HEADER_MAIN_FONT_SIZE = 24; // Increased from 16
const SP_SPACING_AFTER_MAIN_HEADER = 7; // Adjusted from 5
const SP_HEADER_SUB_FONT_SIZE = 14; // Increased from 9
const SP_SPACING_AFTER_SUB_HEADER = 6; // Adjusted from 5
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const drawFillInRow = (labelEn: string, value: string, labelAr: string, options: {valueFontSize?: number, labelEnFontSize?: number, labelArFontSize?: number, valueAlign?: 'left'|'center'|'right', isQtyRow?: boolean} = {}) => {
                ...
                doc.text(labelEn.toUpperCase(), labelEnX, textBaselineY, { baseline: 'middle' }); // Uppercase label
            ...
            const forEnText = `FOR ${forEnRaw}`; // Uppercase and no colon
            ...
            const drawSectionHeader = (titleEn: string, titleAr: string) => {
                ...
                doc.setTextColor(TEXT_COLOR_HEADER[0], TEXT_COLOR_HEADER[1], TEXT_COLOR_HEADER[2]);
                doc.text(titleEn.toUpperCase(), PAGE_MARGIN_X, sectionHeaderTextY , { baseline: 'middle' }); // Uppercase title
            ...
            const drawServiceCheckboxRowPdf = (serviceEnMasterLabel: string, serviceArMasterLabel: string, checked: boolean) => {
                ...
                const serviceEnText = serviceEnMasterLabel.toUpperCase(); // Already uppercase but ensure
            ...
            adZones.forEach((zone, index) => {
                const zoneLabelWidth = doc.getStringUnitWidth(zone.label) * adZoneFontSize / doc.internal.scaleFactor; // zone.label is now "ZONE A"
            ...
            doc.setTextColor(TEXT_COLOR_LABEL_EN[0], TEXT_COLOR_LABEL_EN[1], TEXT_COLOR_LABEL_EN[2]);
            doc.text("THIS RECEIPT IS NOT A TAX INVOICE.", PAGE_MARGIN_X, disclaimerTextY, {baseline: 'middle'}); // Uppercase
            doc.setFont(effectiveFontAr, 'normal');
            doc.setFontSize(SP_DISCLAIMER_FONT_SIZE); 
            ...
            const drawSignature = (centerX: number, nameValue: string, labelEn: string, labelAr: string) => {
                ...
                doc.setTextColor(TEXT_COLOR_LABEL_EN[0], TEXT_COLOR_LABEL_EN[1], TEXT_COLOR_LABEL_EN[2]);
                doc.text(labelEn.toUpperCase(), centerX, sigLabelNameY, { align: 'center', baseline: 'alphabetic' }); // Uppercase label
            ...
            drawSignature(receiverSigCenterX, details.receiverName, "RECEIVER'S SIGNATURE", "المستلم");

            const payerSigCenterX = pageWidth - PAGE_MARGIN_X - (contentWidth * 0.25);
            drawSignature(payerSigCenterX, details.payerName, "SIGNATURE", "الإمضاء");
            

            const now = new Date();
            const filename = `Receipt-${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getDate().toString().padStart(2, '0')}_${now.getHours().toString().padStart(2, '0')}-${now.getMinutes().toString().padStart(2, '0')}-${now.getSeconds().toString().padStart(2, '0')}.pdf`;
